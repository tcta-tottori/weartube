...
class SearchRepository(
    ...
    /**
     * URL 入力で追加するときにタイトルなどを補う。キーが無い・失敗したときは ID だけの [VideoItem] を返す
     * （タイトルは時計のプレーヤーが再生時に補う）。
     */
    suspend fun details(videoId: String): VideoItem {
        val key = settings.currentApiKey()
        if (!key.isSet || quotaExceeded) return VideoItem.fromId(videoId)
        return try {
            api
                .videos(id = videoId, apiKey = key.key)
                .items
                .firstOrNull()
                ?.toVideoItem() ?: VideoItem.fromId(videoId)
        } catch (e: HttpException) {
            classify(e)
            VideoItem.fromId(videoId)
        } catch (e: IOException) {
            VideoItem.fromId(videoId)
        }
    }
